...
static void gfx_opengl_log_info(void) {
    ...
    printf("GL: version: %s", version ? version : "unknown");
    printf("GL: vendor: %s", vendor ? vendor : "unknown");
    printf("GL: renderer: %s", renderer ? renderer : "unknown");
    printf("GL: GLSL version: %s", glsl_version ? glsl_version : "unknown");
    printf("GL: ARB_framebuffer_object: %s", gfx_opengl_supports_framebuffers() ? "yes" : "no");
    printf("GL: ARB_depth_clamp: %s", GLAD_GL_ARB_depth_clamp ? "yes" : "no");
    printf("GL: ARB_texture_mirror_clamp_to_edge: %s", GLAD_GL_ARB_texture_mirror_clamp_to_edge ? "yes" : "no");
}
...
static void *gl_load_proc(const char *name) {
    ...
    printf("GL: could not find function: %s", name);
...
static void gfx_opengl_init(void) {
    ...
    if (GLVersion.major < 2 || (GLVersion.major == 2 && GLVersion.minor < 1)) {
        ...
        printf("Could not load OpenGL 2.1.\nReported version: %d.%d (%s)",
            GLVersion.major, GLVersion.minor, ver ? ver : "unknown");
    }

    if (!gfx_opengl_supports_shaders()) {
        printf("GL: GLSL 1.30 unsupported");
        // maybe replace this with printf, though the GLSL compiler will cause that later anyway
    }

    if (!gfx_framebuffers_enabled) {
        printf("GL: framebuffer effects disabled by user");
    } else if (!gfx_opengl_supports_framebuffers()) {
        printf("GL: GL_ARB_framebuffer_object unsupported, framebuffer effects disabled");
    ...
    if (gl_core_profile) {
        ...
        printf("GL: using core profile, watch out for errors");

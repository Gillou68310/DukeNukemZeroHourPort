            ...
            for (int j = 0; j < 4; j++) {
                ...
                switch (c[i][0][j]) {
                    ...
                    case G_CCMUX_COMBINED:
                        ...
                        break;
                    default:
                        printf("Unsupported ccmux: %d", c[i][0][j]);
                        break;
                }
...
static void import_texture(int i, int tile, bool importReplacement) {
    ...
    if (fmt == G_IM_FMT_RGBA) {
        if (siz == G_IM_SIZ_16b) {
            ...
            printf("Bad size for RGBA texture in tile %d: %02x", tile, siz);
        }
        ...
        if (siz == G_IM_SIZ_4b) {
            ...
            printf("Bad size for IA texture in tile %d: %02x", tile, siz);
        }
        ...
        if (siz == G_IM_SIZ_4b) {
            ...
            printf("Bad size for CI texture in tile %d: %02x", tile, siz);
        }
        ...
        if (siz == G_IM_SIZ_4b) {
            ...
            printf("Bad size for I texture in tile %d: %02x", tile, siz);
        }
    } else {
        printf("Bad texture format in tile %d: %02x %02x", tile, fmt, siz);
    }
}
...
static void gfx_sp_vertex(size_t n_vertices, size_t dest_index, const Vtx* vertices) {
    ...
    for (size_t i = 0; i < n_vertices; i++, dest_index++) {
        ...
        d->color.a = v->cn[3]; // can be required for SHADE_ALPHA even if fog is enabled
    }
}
...
extern "C" void gfx_init(struct GfxWindowManagerAPI* wapi, struct GfxRenderingAPI* rapi, const char* game_name,
    ...
    if (gfx_msaa_level > 1 && !gfx_framebuffers_enabled) {
        printf("F3D: MSAA set to %d, but framebuffers are not available; disabling", gfx_msaa_level);
        gfx_msaa_level = 1;
    }
